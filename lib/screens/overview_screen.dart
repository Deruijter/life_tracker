...
class _OverviewScreenState extends State<OverviewScreen> {
  ...
  _loadTrackers() async {
    ...
    List<Tracker> trackersList = await TrackerRepository.instance
        .getTrackersWithOccurrencesByDate(startDate, endDate);
    setState(() {
    ...
    switch (tracker.type) {
      ...
      case TrackerType.timer:
        DateTime latestOccurrence =
            await TrackerService().addOccurrenceTimer(tracker);
        if (tracker is TimerTracker) {
          // Check & cast Tracker to TimerTracker
      ...
      case TrackerType.monitor:
        ...
        valueText =
            valueText.replaceAll(',', '.'); // incase the user input commas
  ...
  void _refreshTracker(Tracker tracker) async {
    ...
    DateTime tomorrow = DateTime(now.year, now.month, now.day + 1);
    String endDate = DateFormat('yyyy-MM-dd').format(tomorrow) + ' 00:00:00';
    int newOccurrenceCount = await TrackerRepository.instance
        .getTrackerOccurrencesByDate(trackerId, startDate, endDate);
  ...
  void _decrementStopOccurrence(Tracker tracker) async {
    switch (tracker.type) {
      ...
      case TrackerType.timer:
        DateTime endTime =
            await TrackerService().addOccurrenceTimerEnd(tracker);
        if (tracker is TimerTracker) {
          // Cast Tracker to TimerTracker
          ...
          tracker.durationFinished = tracker.durationFinished +
              ((durationEndTime.difference(durationStartTime).inSeconds) / 60);
        }
      default:
    }
    _refreshTracker(tracker);
  ...
  void _getTrackerDetails(int trackerId) async {
    TrackerDetails? trackerDetails =
        await TrackerRepository.instance.getTrackerDetails(trackerId);

    setState(() {
      Navigator.pushNamed(context, '/trackerDetails',
          arguments: {'trackerDetails': trackerDetails});
    });
  }

  @override
  Widget build(BuildContext context) {
    // Let's load the trackers each time we load the widget, 
    // so we have correct info when switching to a new day, 
    //or the user made changes on another screen
    _loadTrackers();  
