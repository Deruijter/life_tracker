...
class CreateEditTrackerScreen extends StatefulWidget {
  ...
  @override
  _CreateEditTrackerScreenState createState() => _CreateEditTrackerScreenState();
}
...
class _CreateEditTrackerScreenState extends State<CreateEditTrackerScreen> {
  ...
  _CreateEditTrackerScreenState()
      : _selectedType = TrackerType.counter;
  ...
  void _saveTracker() async{
    ...
    if(_selectedType == TrackerType.timer || _selectedType == TrackerType.text) {
      trackerUnit = '';
    }
    if (_isEditMode) {
  ...
  void _deleteTracker() async{
    if (_isEditMode) {
      await TrackerRepository.instance.deleteTrackerAndOccurrences(
        widget.initialTracker!.id
      );
    }
  }
       ...
       children: <Widget>[
        SizedBox(
         ...
         Padding(
          ...
          if(_selectedType == TrackerType.counter || _selectedType == TrackerType.monitor)
         ...
         ElevatedButton(
           ...
           child: const Text('Save Trackers'),
         ),
         if(_isEditMode) 
          ElevatedButton (
            onPressed: ()async {
